...
mod tests {
    ...
    fn regression_rs_width() {
        let tester = ScalarUdfTester::new(
            crate::rs_size::rs_width_udf().into(),
            vec![RASTER],
        );
        let rasters = standard_rasters();
    ...
    fn regression_rs_height() {
        let tester = ScalarUdfTester::new(
            crate::rs_size::rs_height_udf().into(),
            vec![RASTER],
        );
        let rasters = standard_rasters();
        let expected: Arc<dyn Array> =
            Arc::new(UInt64Array::from(vec![Some(2), None, Some(4)]));
        let result = tester.invoke_array(Arc::new(rasters)).unwrap();
        assert_array_equal(&result, &expected);
    }
    ...
    fn regression_rs_numbands() {
        let tester = ScalarUdfTester::new(
            crate::rs_numbands::rs_numbands_udf().into(),
            vec![RASTER],
        );
    ...
    fn regression_rs_bandpixeltype() {
        let tester = ScalarUdfTester::new(
            crate::rs_band_accessors::rs_bandpixeltype_udf().into(),
            vec![RASTER],
        );
    ...
    fn regression_rs_bandnodatavalue() {
        let tester = ScalarUdfTester::new(
            crate::rs_band_accessors::rs_bandnodatavalue_udf().into(),
            vec![RASTER],
        );
        let rasters = standard_rasters();
        let expected: Arc<dyn Array> =
    ...
    fn regression_rs_bandpath_indb() {
        let tester = ScalarUdfTester::new(
            crate::rs_bandpath::rs_bandpath_udf().into(),
            vec![RASTER],
        );
    ...
    fn regression_rs_upperleftx() {
        let tester = ScalarUdfTester::new(
            crate::rs_geotransform::rs_upperleftx_udf().into(),
            vec![RASTER],
        );
        let rasters = standard_rasters();
        let expected: Arc<dyn Array> =
    ...
    fn regression_rs_upperlefty() {
        let tester = ScalarUdfTester::new(
            crate::rs_geotransform::rs_upperlefty_udf().into(),
            vec![RASTER],
        );
        let rasters = standard_rasters();
        let expected: Arc<dyn Array> =
    ...
    fn regression_rs_scalex() {
        let tester = ScalarUdfTester::new(
            crate::rs_geotransform::rs_scalex_udf().into(),
            vec![RASTER],
        );
        let rasters = standard_rasters();
        let expected: Arc<dyn Array> =
            Arc::new(Float64Array::from(vec![Some(0.1), None, Some(0.2)]));
        let result = tester.invoke_array(Arc::new(rasters)).unwrap();
        assert_array_equal(&result, &expected);
    }
    ...
    fn regression_rs_scaley() {
        let tester = ScalarUdfTester::new(
            crate::rs_geotransform::rs_scaley_udf().into(),
            vec![RASTER],
        );
        let rasters = standard_rasters();
        let expected: Arc<dyn Array> =
            Arc::new(Float64Array::from(vec![Some(-0.2), None, Some(-0.4)]));
        let result = tester.invoke_array(Arc::new(rasters)).unwrap();
        assert_array_equal(&result, &expected);
    }
    ...
    fn regression_rs_skewx() {
        let tester = ScalarUdfTester::new(
            crate::rs_geotransform::rs_skewx_udf().into(),
            vec![RASTER],
        );
        let rasters = standard_rasters();
        let expected: Arc<dyn Array> =
            Arc::new(Float64Array::from(vec![Some(0.0), None, Some(0.06)]));
        let result = tester.invoke_array(Arc::new(rasters)).unwrap();
        assert_array_equal(&result, &expected);
    }
    ...
    fn regression_rs_skewy() {
        let tester = ScalarUdfTester::new(
            crate::rs_geotransform::rs_skewy_udf().into(),
            vec![RASTER],
        );
        let rasters = standard_rasters();
        let expected: Arc<dyn Array> =
            Arc::new(Float64Array::from(vec![Some(0.0), None, Some(0.08)]));
        let result = tester.invoke_array(Arc::new(rasters)).unwrap();
        assert_array_equal(&result, &expected);
    }
    ...
    fn regression_rs_rotation() {
        let tester = ScalarUdfTester::new(
            crate::rs_geotransform::rs_rotation_udf().into(),
            vec![RASTER],
        );
        let rasters = standard_rasters();
    ...
    fn regression_rs_rastertoworldcoordx() {
        let tester = ScalarUdfTester::new(
            crate::rs_worldcoordinate::rs_rastertoworldcoordx_udf().into(),
            vec![
                RASTER,
    ...
    fn regression_rs_rastertoworldcoordy() {
        let tester = ScalarUdfTester::new(
            crate::rs_worldcoordinate::rs_rastertoworldcoordy_udf().into(),
            vec![
                RASTER,
                SedonaType::Arrow(DataType::Int32),
                SedonaType::Arrow(DataType::Int32),
    ...
    fn regression_rs_rastertoworldcoord() {
        let tester = ScalarUdfTester::new(
            crate::rs_worldcoordinate::rs_rastertoworldcoord_udf().into(),
            vec![
                RASTER,
                SedonaType::Arrow(DataType::Int32),
                SedonaType::Arrow(DataType::Int32),
    ...
    fn regression_rs_worldtorastercoordx() {
        let tester = ScalarUdfTester::new(
            crate::rs_rastercoordinate::rs_worldtorastercoordx_udf().into(),
            vec![
                RASTER,
    ...
    fn regression_rs_worldtorastercoordy() {
        let tester = ScalarUdfTester::new(
            crate::rs_rastercoordinate::rs_worldtorastercoordy_udf().into(),
            vec![
                RASTER,
                SedonaType::Arrow(DataType::Float64),
                SedonaType::Arrow(DataType::Float64),
    ...
    fn regression_rs_srid() {
        let tester = ScalarUdfTester::new(
            crate::rs_srid::rs_srid_udf().into(),
            vec![RASTER],
        );
        let rasters = standard_rasters();
    ...
    fn regression_rs_georeference() {
        let tester = ScalarUdfTester::new(
            crate::rs_georeference::rs_georeference_udf().into(),
            vec![RASTER],
        );
        let rasters = standard_rasters();
        let result = tester.invoke_array(Arc::new(rasters)).unwrap();
    ...
    fn regression_rs_envelope() {
        let tester = ScalarUdfTester::new(
            crate::rs_envelope::rs_envelope_udf().into(),
            vec![RASTER],
        );
        let rasters = standard_rasters();
        let result = tester.invoke_array(Arc::new(rasters)).unwrap();
    ...
    fn regression_rs_convexhull() {
        let tester = ScalarUdfTester::new(
            crate::rs_convexhull::rs_convexhull_udf().into(),
            vec![RASTER],
        );
        let rasters = standard_rasters();
        let result = tester.invoke_array(Arc::new(rasters)).unwrap();
    ...
    fn regression_rs_pixelaspoint() {
        let tester = ScalarUdfTester::new(
            crate::rs_pixel_functions::rs_pixelaspoint_udf().into(),
            vec![
                RASTER,
                SedonaType::Arrow(DataType::Int32),
                SedonaType::Arrow(DataType::Int32),
    ...
    fn regression_rs_pixelascentroid() {
        let tester = ScalarUdfTester::new(
            crate::rs_pixel_functions::rs_pixelascentroid_udf().into(),
            vec![
                RASTER,
                SedonaType::Arrow(DataType::Int32),
                SedonaType::Arrow(DataType::Int32),
    ...
    fn regression_rs_pixelaspolygon() {
        let tester = ScalarUdfTester::new(
            crate::rs_pixel_functions::rs_pixelaspolygon_udf().into(),
            vec![
                RASTER,
                SedonaType::Arrow(DataType::Int32),
                SedonaType::Arrow(DataType::Int32),
    ...
    fn regression_rs_intersects() {
        ...
        let tester = ScalarUdfTester::new(
            crate::rs_spatial_predicates::rs_intersects_udf().into(),
        ...
        let result = tester
            .invoke_arrays(vec![Arc::new(rasters), geom])
            .unwrap();
    ...
    fn regression_rs_contains() {
        let geom_type = SedonaType::Wkb(Edges::Planar, lnglat());
        let tester = ScalarUdfTester::new(
            crate::rs_spatial_predicates::rs_contains_udf().into(),
            vec![RASTER, geom_type.clone()],
        );
        let rasters = generate_test_rasters(3, Some(0)).unwrap();
    ...
    fn regression_rs_setsrid_roundtrip() {
        let set_tester = ScalarUdfTester::new(
            crate::rs_setsrid::rs_set_srid_udf().into(),
    ...
    fn regression_rs_example() {
        ...
        let udf: datafusion_expr::ScalarUDF = crate::rs_example::rs_example_udf().into();
    ...
    fn regression_null_propagation() {
        ...
        let functions: Vec<(&str, Box<dyn Fn() -> ScalarUdfTester>)> = vec![
            ("rs_width", Box::new(|| ScalarUdfTester::new(crate::rs_size::rs_width_udf().into(), vec![RASTER]))),
            ("rs_height", Box::new(|| ScalarUdfTester::new(crate::rs_size::rs_height_udf().into(), vec![RASTER]))),
            ("rs_numbands", Box::new(|| ScalarUdfTester::new(crate::rs_numbands::rs_numbands_udf().into(), vec![RASTER]))),
            ("rs_upperleftx", Box::new(|| ScalarUdfTester::new(crate::rs_geotransform::rs_upperleftx_udf().into(), vec![RASTER]))),
            ("rs_upperlefty", Box::new(|| ScalarUdfTester::new(crate::rs_geotransform::rs_upperlefty_udf().into(), vec![RASTER]))),
            ("rs_scalex", Box::new(|| ScalarUdfTester::new(crate::rs_geotransform::rs_scalex_udf().into(), vec![RASTER]))),
            ("rs_scaley", Box::new(|| ScalarUdfTester::new(crate::rs_geotransform::rs_scaley_udf().into(), vec![RASTER]))),
            ("rs_skewx", Box::new(|| ScalarUdfTester::new(crate::rs_geotransform::rs_skewx_udf().into(), vec![RASTER]))),
            ("rs_skewy", Box::new(|| ScalarUdfTester::new(crate::rs_geotransform::rs_skewy_udf().into(), vec![RASTER]))),
            ("rs_rotation", Box::new(|| ScalarUdfTester::new(crate::rs_geotransform::rs_rotation_udf().into(), vec![RASTER]))),
            ("rs_srid", Box::new(|| ScalarUdfTester::new(crate::rs_srid::rs_srid_udf().into(), vec![RASTER]))),
            ("rs_crs", Box::new(|| ScalarUdfTester::new(crate::rs_srid::rs_crs_udf().into(), vec![RASTER]))),
        ];
