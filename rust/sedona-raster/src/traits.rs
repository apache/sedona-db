...
pub struct BandOverrides<'a> {
    /// Name for the derived band. A `BandRef` carries no source name to
    /// inherit (band names live at the raster level), so both `Keep` and
    /// `Clear` yield an unnamed band; `Set(n)` names it.
...
pub trait BandRef {
    ...
    ///
    /// Persistence is delegated to [`RasterBuilder::start_band`]: an identity
    /// effective view is stored as the canonical null sentinel, and a
    /// non-identity one (a slice, broadcast, permutation, or reverse) is
    /// persisted explicitly and decoded back by the reader. The source bytes
    /// are carried over unchanged — the view relocates the visible window over
    /// them, it does not repack them.
    fn copy_into(
        &self,
        ...
        let effective_view: Vec<ViewEntry> = match overrides.view {
            Override::Keep => self.view().to_vec(),
            Override::Clear => ViewEntries::identity_for_shape(&source_shape)
                .as_slice()
                .to_vec(),
            Override::Set(v) => v.to_vec(),
        };
...
mod tests {
    ...
    fn copy_into_set_view_uses_it_as_is() {
        ...
        assert_eq!(out_band.view(), &[ve(0, 1, 1, 3)]);
        assert_eq!(out_band.shape(), &[3]);
        assert_eq!(out_band.raw_source_shape(), &[8]);
        let buf = out_band.nd_buffer().unwrap();
    ...
    fn copy_into_caller_composed_view() {
        ...
        let composed = ViewEntries::new(in_band.view().to_vec())
            .compose(&next)
            .unwrap();
        ...
        assert_eq!(out_band.view(), &[ve(0, 1, 2, 2)]);
    ...
    fn copy_into_clear_view_resets_to_identity() {
        ...
        assert_eq!(in_band.view(), &[ve(0, 1, 2, 3)]);
        ...
        assert_eq!(out_band.view(), &[ve(0, 0, 1, 8)]);
