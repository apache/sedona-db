...
use crate::{DistanceExt, GeoFloat};
    ...
    T: GeoFloat,
{
    ...
    let (farthest_index, farthest_distance) = rdp_indices
        ...
        .map(|(index, rdp_index)| {
            let dist = rdp_index.coord.distance_ext(&first_last_line);
            (index, dist)
        })
        .fold(
            (0usize, T::zero()),
...
pub trait Simplify<T, Epsilon = T> {
    ...
    ///
    /// ```
    /// use sedona_geo_generic_alg::Simplify;
...
pub trait SimplifyIdx<T, Epsilon = T> {
    ...
    ///
    /// ```
    /// use sedona_geo_generic_alg::SimplifyIdx;
    /// use sedona_geo_generic_alg::line_string;
    ///
    /// let line_string = line_string![
    ///     (x: 0.0, y: 0.0),
