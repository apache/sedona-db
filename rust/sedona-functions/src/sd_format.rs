...
fn sedona_type_to_formatted_type(sedona_type: &SedonaType) -> Result<SedonaType> {
    match sedona_type {
        ...
        SedonaType::Raster => internal_err!(
            "Formatting a raster value requires sedona-raster-functions to be registered; \
             this UDF cannot format a raster value nested inside a struct or list column"
        ),
    }
}
...
mod tests {
    ...
    #[test]
    fn sd_format_errors_on_raster_nested_in_struct() {
        use sedona_testing::rasters::generate_multi_band_raster;

        let geom_values = vec![Some("POINT(1 2)")];
        let geom_array = create_array(&geom_values, &WKB_GEOMETRY);

        let raster_field = SedonaType::Raster.to_storage_field("raster", true).unwrap();
        let raster_array: ArrayRef = Arc::new(generate_multi_band_raster());

        let struct_fields = vec![
            Arc::new(WKB_GEOMETRY.to_storage_field("geom", true).unwrap()),
            Arc::new(raster_field),
        ];
        let struct_array = StructArray::new(
            struct_fields.clone().into(),
            vec![geom_array, raster_array],
            None,
        );
        ...
        let err = tester.invoke_array(Arc::new(struct_array)).unwrap_err();
        assert!(
            err.message().contains("sedona-raster-functions"),
            "unexpected error: {}",
            err.message()
        );
    }
    ...
    #[test]
    fn sd_format_errors_on_raster_only_struct() {
        use sedona_testing::rasters::generate_multi_band_raster;

        let raster_field = SedonaType::Raster.to_storage_field("raster", true).unwrap();
        let raster_array: ArrayRef = Arc::new(generate_multi_band_raster());

        let struct_fields = vec![Arc::new(raster_field)];
        let struct_array = StructArray::new(struct_fields.clone().into(), vec![raster_array], None);
