...
impl SedonaScalarKernel for STXyzm {
    ...
    fn invoke_batch(
        ...
        let mut null_buffer: Vec<bool> = vec![];
        let mut bytes_buffer: Vec<&[u8]> = vec![];
        executor.execute_wkb_void(|maybe_item| {
            match maybe_item {
                Some(item) => {
                    // builder.append_option(invoke_scalar(&item, dim_index)?);
                    // minimum length of a WKB point is 1 + 4 + 8 + 8 = 21
                    use datafusion_common::exec_err; // TODO: move up
                    if item.len() < 21 {
                        return exec_err!("Invalid WKB point of length {}", item.len());
                    }
                    let byte_order = item[0];
                    let geom_type = match byte_order {
                        0 => u32::from_be_bytes([item[1], item[2], item[3], item[4]]),
                        1 => u32::from_le_bytes([item[1], item[2], item[3], item[4]]),
                        _ => exec_err!("Invalid byte order: {byte_order}")?,
                    };
                    // If not a point, return null
                    let is_null = if geom_type != 1 { true } else { false };
                    null_buffer.push(is_null);
                }
                None => null_buffer.push(true),
            }
            Ok(())
        })?;
