...
mod tests {
    ...
    fn band_fragment_extracts_index() {
        assert_eq!(
            parse_ok("s3://bucket/file.tif#band=42"),
    ...
    fn band_one_fragment_round_trips() {
        assert_eq!(
            parse_ok("s3://bucket/file.tif#band=1"),
            ("s3://bucket/file.tif".to_string(), 1),
        );
    }
    ...
    fn band_max_u32_accepted() {
        ...
        assert_eq!(parse_ok(&uri), ("s3://bucket/file.tif".to_string(), max));
    }

    #[test]
    fn band_zero_errors() {
        let msg = parse_err_msg("s3://bucket/file.tif#band=0");
    ...
    fn negative_band_errors() {
        let msg = parse_err_msg("s3://bucket/file.tif#band=-2");
    ...
    fn band_overflow_errors() {
        ...
        let msg = parse_err_msg("s3://bucket/file.tif#band=4294967296");
        assert!(msg.contains("band=4294967296"), "msg was: {msg}");
    }

    #[test]
    fn non_numeric_band_errors() {
        let msg = parse_err_msg("s3://bucket/file.tif#band=abc");
        assert!(msg.contains("band=abc"), "msg was: {msg}");
    }

    #[test]
    fn empty_band_value_errors() {
        let msg = parse_err_msg("s3://bucket/file.tif#band=");
    ...
    fn non_band_fragment_passes_through() {
        ...
        assert_eq!(parse_ok(uri), (uri.to_string(), 1));
    }
    ...
    fn url_query_string_preserved_with_band_fragment() {
        assert_eq!(
            parse_ok("https://example.com/r.tif?token=abc#band=3"),
    ...
    fn local_path_without_fragment() {
        assert_eq!(parse_ok("/tmp/file.tif"), ("/tmp/file.tif".to_string(), 1));
    }
    ...
    fn gdal_subdataset_with_band_fragment_extracts_band() {
        ...
        let (gdal_uri, band) = parse_ok(uri);
    ...
    fn trailing_band_wins_over_earlier_anchor() {
        assert_eq!(
            parse_ok("https://example.com/r.tif#anchor#band=7"),
    ...
    #[test]
    fn empty_uri() {
        assert_eq!(parse_ok(""), (String::new(), 1));
    }

    #[test]
    fn returns_borrowed_for_band_fragment() {
        let uri = "s3://bucket/file.tif#band=2".to_string();
        let (out, _) = parse_outdb_source(&uri).unwrap();
        assert!(matches!(out, Cow::Borrowed(_)));
    }

    #[test]
    fn returns_borrowed_for_passthrough() {
        let uri = "s3://bucket/file.tif".to_string();
        let (out, _) = parse_outdb_source(&uri).unwrap();
        assert!(matches!(out, Cow::Borrowed(_)));
    }
}
