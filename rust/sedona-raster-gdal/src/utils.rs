...
//! Utility functions for loading raster data via GDAL into the canonical
//! N-D Arrow schema.
...
///
/// All bands are written as 2-D `["y", "x"]` bands with the canonical identity
/// view. Band data is read from GDAL as a contiguous row-major byte buffer.
pub fn append_as_indb_raster(dataset: &Dataset, builder: &mut RasterBuilder) -> Result<()> {
    ...
    let [origin_x, scale_x, skew_x, origin_y, skew_y, scale_y] = geotransform;
    ...
    let width_u64 = u64::try_from(width)
        .map_err(|_| exec_datafusion_err!("Raster width {} does not fit in u64", width))?;
    let height_u64 = u64::try_from(height)
        .map_err(|_| exec_datafusion_err!("Raster height {} does not fit in u64", height))?;
...
mod tests {
    ...
    fn dataset_to_indb_raster_reads_single_band_geotiff() {
        ...
        assert_eq!(&*band.contiguous_data().unwrap(), &[1u8, 2, 3, 4, 5, 6]);
    }
    ...
    fn dataset_to_indb_raster_preserves_multi_band_data_and_nodata() {
        ...
        assert_eq!(&*band1.contiguous_data().unwrap(), &[10u8, 11, 12, 13]);

        assert!(band2.is_indb());
        assert_eq!(band2.data_type(), BandDataType::UInt8);
        assert_eq!(band2.nodata().unwrap(), [255u8]);
        assert_eq!(&*band2.contiguous_data().unwrap(), &[100u8, 0, 200, 0]);
    }
