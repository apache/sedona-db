...
```sql
SELECT RS_Tile(RS_Example(), 32, 16);
```

Tile every band, padding the edge tiles to the full 40x20 size with a
nodata fill of 0:

```sql
SELECT RS_Tile(RS_Example(), 40, 20, true, 0.0);
```

Tile an ordered subset of bands:

```sql
SELECT RS_Tile(RS_Example(), make_array(1, 3), 32, 16);
```
