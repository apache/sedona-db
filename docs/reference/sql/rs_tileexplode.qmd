...
kernels:
  ...
  - returns: list
    ...
    - name: band_index
      type: integer
      description: 1-based index of the single band to include in each tile.
    - name: width
      type: integer
      description: Tile width in pixels. Must be >= 1.
    - name: height
...
`RS_TileExplode` cuts a raster into a grid of `width` x `height` tiles,
returning a list with one item per tile. It mirrors Sedona Spark's
...
It has three overloads, mirroring Sedona Spark verbatim (`padWithNoData` and
`noDataVal` are trailing optionals):

- `RS_TileExplode(raster, width, height, padWithNoData = false, noDataVal = null)`
- `RS_TileExplode(raster, bandIndex, width, height, padWithNoData = false, noDataVal = null)`
- `RS_TileExplode(raster, bandIndices, width, height, padWithNoData = false, noDataVal = null)`

When no band argument is given, every band is included. `bandIndex` selects a
single 1-based band; `bandIndices` selects an ordered subset (equivalent to
`bandIndex` with a one-element array). Band indices are 1-based.
...
Tile every band into a 2x2 grid of 32x16 tiles:

```sql
SELECT RS_TileExplode(RS_Example(), 32, 16);
```

Tile only the first band, padding the edge tiles to the full 40x20 size with a
nodata fill of 0:

```sql
SELECT RS_TileExplode(RS_Example(), 1, 40, 20, true, 0.0);
```

Tile an ordered subset of bands:

```sql
SELECT RS_TileExplode(RS_Example(), make_array(1, 3), 32, 16);
```
