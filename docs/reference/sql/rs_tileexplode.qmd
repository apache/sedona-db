...
returning a list with one item per tile. Because SedonaDB has no per-row
generator, the tiles are returned as a list that you `UNNEST` to get one row
per tile.

It has three overloads (`padWithNoData` and `noDataVal` are trailing
optionals):
