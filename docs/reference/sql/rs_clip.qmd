...
kernels:
  ...
  - returns: raster
    ...
    - name: band
      type: integer
      description: 1-based index of the band to clip.
    - name: geom
      type: geometry
      description: >
...
box, otherwise the original extent is preserved.
...
SELECT RS_Width(
  RS_Clip(
    RS_Example(),
    1,
    ST_SetCRS(ST_GeomFromText('POLYGON ((60 90, 160 90, 160 190, 60 190, 60 90))'), 'OGC:CRS84')
  )
);
```
