...
kernels:
  ...
  - returns: float64
    ...
    - name: colX
      type: int
      description: Pixel column (1-based).
    - name: rowY
      type: int
      description: Pixel row (1-based).
    - name: band
      type: int
...
The pixel can instead be addressed by its grid coordinate. Grid coordinates
are 1-based, as in PostGIS `ST_Value`: `(1, 1)` is the upper-left pixel, and a
column or row of `0` is outside the raster.
...
The upper-left pixel of band 1, by grid coordinate:

```sql
SELECT RS_Value(RS_Example(), 1, 1, 1);
```
