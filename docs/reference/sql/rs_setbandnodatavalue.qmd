---
# Licensed to the Apache Software Foundation (ASF) under one
# or more contributor license agreements.  See the NOTICE file
# distributed with this work for additional information
# regarding copyright ownership.  The ASF licenses this file
# to you under the Apache License, Version 2.0 (the
# "License"); you may not use this file except in compliance
# with the License.  You may obtain a copy of the License at
#
#   http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing,
# software distributed under the License is distributed on an
# "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
# KIND, either express or implied.  See the License for the
# specific language governing permissions and limitations
# under the License.

title: RS_SetBandNoDataValue
description: >
  Sets the nodata value of the specified band, packing the value into the band's
  native data type. A null raster or band yields a null raster; a null nodata
  value instead clears the addressed band's nodata. Metadata-only unless the
  four-argument form asks otherwise: with `replace` true, pixels holding the
  band's old nodata are rewritten to the new one so they keep reading as nodata.
kernels:
  - returns: raster
    args:
    - raster
    - name: nodata
      type: float64
      description: >
        Nodata value to set, or null to clear the band's nodata. The raster must
        be single-band; for a multiband raster, specify the band with the
        three-argument form below.
  - returns: raster
    args:
    - raster
    - name: band
      type: int
      description: Band index (1-based).
    - name: nodata
      type: float64
      description: Nodata value to set on the band, or null to clear it.
  - returns: raster
    args:
    - raster
    - name: band
      type: int
      description: Band index (1-based).
    - name: nodata
      type: float64
      description: Nodata value to set on the band, or null to clear it.
    - name: replace
      type: boolean
      description: >
        When true, every pixel in the band currently equal to its existing
        nodata is rewritten to the new value, so those pixels still read as
        nodata afterwards. The band must already have a nodata value; replacing
        against none is an error. Other bands are left alone. Ignored when
        `nodata` is null, which clears the band and leaves no sentinel to
        rewrite pixels to.
---

## Examples

```sql
SELECT RS_BandNoDataValue(RS_SetBandNoDataValue(RS_Example(), 1, 0), 1);
```

```sql
SELECT RS_BandNoDataValue(RS_SetBandNoDataValue(RS_Example(), 2, 255), 2);
```

A null nodata value clears the band's existing nodata, so the getter reads back null:

```sql
SELECT RS_BandNoDataValue(RS_SetBandNoDataValue(RS_Example(), 1, CAST(NULL AS DOUBLE)), 1);
```

With `replace`, the pixels that held the old nodata are rewritten to the new one,
so the count of nodata pixels survives the change:

```sql
SELECT RS_BandNoDataValue(
  RS_SetBandNoDataValue(RS_SetBandNoDataValue(RS_Example(), 1, 0), 1, 255, true),
  1
);
```
