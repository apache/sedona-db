...
use crate::vsi;
use crate::vsi::{VsiDir, VsiDirEntry};
...
impl Gdal {
    ...
    pub fn open_vsi_dir(
        ...
        vsi::open_dir(self.api, path, recurse_depth, options)
    }

    /// Return the directory separator used by GDAL for a given path.
    pub fn vsi_directory_separator(&self, path: &str) -> Result<String> {
        vsi::get_directory_separator(self.api, path)
    }

    /// Read all VSI entries from a directory handle.
    pub fn read_all_vsi_entries(
        &self,
        path: &str,
        recurse_depth: i32,
        options: Option<&CslStringList>,
    ) -> Result<Vec<VsiDirEntry>> {
        let mut dir = self.open_vsi_dir(path, recurse_depth, options)?;
        Ok(dir.by_ref().collect())
    }
