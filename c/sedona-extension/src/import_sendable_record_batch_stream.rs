...
impl AsyncDeviceStreamHandler {
    ...
    /// Transfer responsibility for releasing this handler to the producer.
    pub fn into_raw(mut self) -> *mut FFI_ArrowAsyncDeviceStreamHandler {
        let ptr = self.ptr;
        self.ptr = null_mut();
        ptr
    }
...
impl ImportedAsyncDeviceStream {
    ...
    pub fn new(prefetch_count: u64) -> (Self, AsyncDeviceStreamHandler) {
        Self::new_inner(prefetch_count, None)
    }

    /// Create a stream whose schema is known by the caller before the producer
    /// invokes `on_schema`.
    ///
    /// DataFusion requires [`RecordBatchStream::schema`] to be available as soon
    /// as `ExecutionPlan::execute` returns, so execution-plan imports use this
    /// constructor with the schema obtained from `SedonaCExecutionPlan`.
    pub fn new_with_schema(
        prefetch_count: u64,
        schema: SchemaRef,
    ) -> (Self, AsyncDeviceStreamHandler) {
        Self::new_inner(prefetch_count, Some(schema))
    }
    ...
    fn new_inner(
        ...
        let stream = Self {
            ...
            receiver,
            schema_struct_type: schema
                .as_ref()
                .map(|schema| DataType::Struct(schema.fields().iter().cloned().collect())),
            schema,
        };
