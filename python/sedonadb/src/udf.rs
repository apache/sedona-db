...
use datafusion_expr::{
    ColumnarValue, ReturnFieldArgs, ScalarFunctionArgs, ScalarUDF, ScalarUDFImpl, Signature, TypeSignature, Volatility
};
use datafusion_ffi::udf::FFI_ScalarUDF;
use pyo3::{
    pyclass, pyfunction, pymethods,
    types::{PyAnyMethods, PyCapsule, PyDict, PyTuple},
...
#[pyfunction]
pub fn register_sedona_scalar_udf<'py>(
    py: Python<'py>,
    name: &str,
    ...
    let volatility = match volatility.unwrap_or("immutable") {
...
impl ScalarUDFImpl for PySedonaScalarUdf {
    ...
    fn return_field_from_args(&self, args: ReturnFieldArgs) -> Result<FieldRef> {
        Ok(eval_return_field(&self.py_return_field, args.arg_fields)?)
    }
...
fn eval_return_field(func: &PyObject, arg_fields: &[FieldRef]) -> Result<FieldRef, PySedonaError> {
    let return_field = Python::with_gil(|py| -> Result<FieldRef, PySedonaError> {
        let py_arg_fields = arg_fields
            .iter()
            .map(|f| PySedonaField::new(f.as_ref().clone()))
            .collect::<Vec<_>>();

        let py_args = PyTuple::new(py, py_arg_fields)?;
        let py_return_field = func.call(py, py_args, None)?;
...
fn eval_invoke_batch(
    ...
    let result = Python::with_gil(|py| -> Result<ArrayRef, PySedonaError> {
        ...
        py_kwargs.set_item("return_field", py_return_field)?;
